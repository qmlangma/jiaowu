    ...
    var body: some View {
        AppCard {
            VStack(alignment: .leading, spacing: 12) {
                StatusBadge(title: "今日任务", tint: priorityTint)
                Text(title)
                    .font(.system(size: 18, weight: .bold))
                Text(detail)
                    .font(.system(size: 14, weight: .medium))
                    .foregroundStyle(JWColor.textMuted)
                SecondaryButton(title: actionTitle, systemImage: "arrow.right", tint: priorityTint, action: action)
            }
        }
    }
}
        ...
        HStack(spacing: 10) {
            ForEach(steps, id: \.self) { step in
                ...
                Text(step)
                    ...
                    .frame(maxWidth: .infinity)
                    .frame(height: 36)
                    .background(selected ? JWColor.primaryLight : JWColor.surfaceMuted)
                    .clipShape(RoundedRectangle(cornerRadius: 10, style: .continuous))
            }
        }
    }
...
struct FeedbackToastView: View {
    ...
    private var tint: Color {
        ...
        case .success: JWColor.success
        case .warning: JWColor.warning
        case .error: JWColor.danger
        }
    }
