...
struct ClassRosterDrawerSheet: View {
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            Spacer(minLength: 0)
            drawerBtn("arrow.up.left.and.arrow.down.right", "全屏")
            drawerBtn("pin", "钉住")
            Button(action: close) {
                HStack(spacing: 7) {
    ...
    private var titleBar: some View {
        Text("2026年 · 春季 · \(session.className ?? "信息学算法") · 小星星")
    ...
    private func drawerBtn(_ symbol: String, _ title: String) -> some View {
        HStack(spacing: 6) {
    ...
    private func signOutArea(student: RosterStudent) -> some View {
        ...
        case let .signedOut(time):
            return AnyView(Text("已签退 \(time)").font(.system(size: 14, weight: .bold)).foregroundStyle(JWColor.success))
        }
    }

    private func transferDetailArea(student: RosterStudent) -> some View {
        let prefix = student.transferOutTag == "转出" ? "转出至：" : "调出至："
        let target = student.transferTarget ?? "待分配班级（第0次）"
